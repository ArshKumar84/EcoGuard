    ...
    <Card>
      ...
      <CardContent>
        ...
        {weatherData && aqi !== null ? (
          ...
          <p>Loading weather and AQI data...</p>
        )}
      </CardContent>
    </Card>
  )
...
const EcoGuardDashboard = () => {
  ...
  const calculateAqiImpact = useCallback((currentAQI, treeCount) => {
    ...
    const AQI_new = currentAQI - (k * N * T) / (A * P)
    return Math.max(0, AQI_new) // Ensure AQI doesn't go negative
  }, [])
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const deforestationResponse = await axios.post('https://ecoguard-model.onrender.com/deforestation', {
        area_name: location
      })
      ...
      // Initialize real-time graph data
  ...
  return (
    <div className="p-4">
      ...
      <div className="mb-4">
        ...
        />
        <Button onClick={fetchData} className="ml-2">Fetch Data</Button>
      </div>
